...
kernels = [
    "gemm"
]  # names of the kernel and folder in the kernels folder (must be the same)
platforms = [
    ("CUDA", "A100")
]  # tuple of language and device, for language choose from CUDA, HIP and OpenCL
    ...
    for searchspace_constructor in searchspace_constructors:
        for kernel in kernels:
            ...
            for language, device in platforms:
                ...
                cachefile_path = Path(
                    f"results/{kernel}/{device.upper()}_f={searchspace_constructor}_i={iteration}.json"
                )
                assert (
                    cachefile_path.parent.exists()
                ), f"Cachefile path {cachefile_path.parent} does not exist"
                if cachefile_path.exists():
                    print(
                        f"    skipping {searchspace_constructor} (iter. {iteration}) as it already exists"
                    )
                ...
                print(
                    f"{searchspace_constructor} (iter. {iteration}) evaluated {len(res)} configs in ~{num_minutes} minutes"
                )

                # assert the cache file path now exists
                assert (
                    cachefile_path.exists()
                ), f"Cachefile does not exist at {cachefile_path}"
    ...
    for kernel in kernels:
        for language, device in platforms:
            for iteration in range(iterations):
                ...
                assert (
                    cachefile_path.exists()
                ), f"Cachefile at {cachefile_path} must exist"
                # get the file created time as a date
                # file_creation_time = datetime.fromtimestamp(cachefile_path.stat().st_mtime)
                with cachefile_path.open("r") as f:
                    data = json.load(f)
                    ...
                    tuning_start_time = datetime.fromisoformat(
                        cache[list(cache.keys())[-1]]["timestamp"]
                    ) - timedelta(minutes=num_minutes)
                    # check if the tuning start time is before the first config
                    assert tuning_start_time < datetime.fromisoformat(
                        next(iter(cache.values()))["timestamp"]
                    ), f"{cachefile_path} tuning start time {tuning_start_time} is not before the first config time"
                    results["tuning_start_time"][
                        searchspace_constructor
                    ] = tuning_start_time
                    results["num_configs"][searchspace_constructor].append(num_configs)
                    configs_performance = [
                        c[performance_objective]
                        for c in cache.values()
                        if performance_objective in c
                        and isinstance(c[performance_objective], (int, float))
                    ]
                    results["configs_performance"][searchspace_constructor].append(
                        configs_performance
                    )
                    ...
                    for k, v in cache.items():
                        ...
                        time_to_config = (
                            config_timestamp - tuning_start_time
                        ).total_seconds() / 60
                        # if we've passed the next point on the minutes line, add the best performance so far
                        if last_time_to_config_index is None or (
                            last_time_to_config_index + 1 < len(minutes_line)
                            and time_to_config
                            >= minutes_line[last_time_to_config_index + 1]
                        ):
                            # get the new last_time_to_config_index it should be set to
                            new_time_to_config_index = np.where(
                                minutes_line >= time_to_config
                            )[0][0]
                            if last_time_to_config_index is None:
                                # if this is the first time we are adding elements, set the performance so far to NaN
                                configs_performance_time_local += [np.nan] * min(
                                    new_time_to_config_index, len(minutes_line)
                                )
                                num_configs_time_local += [np.nan] * min(
                                    new_time_to_config_index, len(minutes_line)
                                )
                                ...
                                elems_to_add = (
                                    new_time_to_config_index - last_time_to_config_index
                                )
                                assert elems_to_add >= 1 and elems_to_add + len(
                                    configs_performance_time_local
                                ) <= len(
                                    minutes_line
                                ), f"{elems_to_add=} + {len(configs_performance_time_local)=} not <= {len(minutes_line)=}"
                                # add the best performance so far for the missing elements
                                configs_performance_time_local += [
                                    best_performance_so_far
                                ] * elems_to_add
                                num_configs_time_local += [
                                    num_configs_so_far
                                ] * elems_to_add
                            ...
                            best_performance_so_far = (
                                min(best_performance_so_far, v[performance_objective])
                                if minimize
                                else max(
                                    best_performance_so_far, v[performance_objective]
                                )
                            )
                    ...
                    remaining_elems_to_add = len(minutes_line) - len(
                        configs_performance_time_local
                    )
                    configs_performance_time_local += [
                        (
                            np.nan
                            if last_time_to_config_index is None
                            else best_performance_so_far
                        )
                    ] * remaining_elems_to_add
                    num_configs_time_local += [
                        (
                            np.nan
                            if last_time_to_config_index is None
                            else num_configs_so_far
                        )
                    ] * remaining_elems_to_add
                    assert len(configs_performance_time_local) == len(
                        minutes_line
                    ), f"{len(configs_performance_time_local)=} != {len(minutes_line)=}"
                    assert len(num_configs_time_local) == len(
                        minutes_line
                    ), f"{len(num_configs_time_local)=} != {len(minutes_line)=}"
                    # check if the performance is monotonically increasing or decreasing (depending on minimize)
                    assert all(
                        x >= y if minimize else x <= y
                        for x, y in zip(
                            configs_performance_time_local[first_time_to_config_index:],
                            configs_performance_time_local[
                                first_time_to_config_index + 1 :
                            ],
                        )
                    )
                    assert all(
                        x <= y
                        for x, y in zip(
                            num_configs_time_local[first_time_to_config_index:],
                            num_configs_time_local[first_time_to_config_index + 1 :],
                        )
                    )
                    results["configs_performance_time"][searchspace_constructor].append(
                        configs_performance_time_local
                    )
                    results["num_configs_time"][searchspace_constructor].append(
                        num_configs_time_local
                    )
...
avg_performance = np.mean(
    np.array(
        [
            ...
            for i in results["configs_performance"][s]
            for c in i
        ]
    ).flatten()
)
...
for s in searchspace_constructors:
    ...
    for i in range(iterations):
        ...
        if minimize:
            results["best_relative_performance"][s].append(
                avg_performance / min(configs_performance)
            )
        else:
            results["best_relative_performance"][s].append(
                max(configs_performance) / avg_performance
            )
...
for s in searchspace_constructors:
    ...
    if first_non_nan_all:
        ...
        assert np.all(
            ~np.isnan(r[:, first_non_nan_index])
        ), f"Not all iterations are non-nan at index {first_non_nan_index} for {s}"
    else:
        # get the first index where any iterations are not NaN
        first_non_nan_index = np.where(~np.isnan(r).all(axis=0))[0][0]
        assert np.any(
            ~np.isnan(r[:, first_non_nan_index])
        ), f"Not any iterations are non-nan at index {first_non_nan_index} for {s}"
    ...
    mean_performance = np.concatenate(
        (np.full(first_non_nan_index, np.nan), mean_performance)
    )
    std_performance = np.concatenate(
        (np.full(first_non_nan_index, np.nan), std_performance)
    )
...
for i, method in enumerate(
    [searchspace_methods_displaynames[s] for s in searchspace_constructors]
):
    plt.plot(
        minutes_line,
        mean_performances[i],
...
plt.tight_layout()
plt.savefig(
    f"compare_real_world_performance_over_time_{kernel}_presentation.png",
    dpi=300,
    bbox_inches="tight",
    pad_inches=0.01,
)
