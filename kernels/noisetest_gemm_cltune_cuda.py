...
def tune(inputs, backends, device=0, no_registers=False, no_L2_flush=False, recopy_arrays=False):
    ...
    for backend in backends:
        filename = f"outputdata/gemm_cltune_cuda/{'flush_L2/' if not no_L2_flush else ''}{'recopy_arrays/' if recopy_arrays else ''}{'no_flush_L2/' if (no_L2_flush and not recopy_arrays) else ''}{'no_registers/' if no_registers else ''}gemm_cltune_cuda_{device_name}_size-{m}x{n}x{k}_noisetest_backend-{backend}_CUDA-{cuda_version}_memset-uint8"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-b", "--backends", nargs="+", default=["CUPY", "CUDA", "NVCUDA"], required=False
    )
    parser.add_argument(
