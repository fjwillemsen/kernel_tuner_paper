...
#if SA == 1
INLINE_FUNC void GlobalToLocalA(const __global realM* restrict agm, realM* alm,
...
#if SB == 1
INLINE_FUNC void GlobalToLocalB(const __global realN* restrict bgm, realN* blm,
...
#if SA == 1
INLINE_FUNC void LocalToPrivateA(realM* alm, realM apm[MWI/VWM], const int kg) {
  #pragma unroll
  for (int mi=0; mi<MWI/VWM; ++mi) {
    #if STRM == 0
      int mg = mi + get_local_id(0)*(MWI/VWM);
...
// Same as above, but now for the B input matrix
#if SB == 1
INLINE_FUNC void LocalToPrivateB(realN* blm, realN bpm[NWI/VWN], const int kg) {
  #pragma unroll
  for (int ni=0; ni<NWI/VWN; ++ni) {
    #if STRN == 0
      int ng = ni + get_local_id(1)*(NWI/VWN);
