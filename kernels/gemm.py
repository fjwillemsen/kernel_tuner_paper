...
#from kernel_tuner.observers import PowerSensorObserver
...
def tune(inputs, pwr_limit=None, device=0):
    ...
    nvmlobserver = NVMLObserver(["nvml_energy", "temperature", "core_freq", "mem_freq"],
                                 save_all=True, nvidia_smi_fallback="/cm/shared/package/utils/bin/run-nvidia-smi", use_locked_clocks=False)  #/cm/shared/package/utils/bin/run-nvidia-smi
    ...
    results, env = kernel_tuner.tune_kernel("Xgemm", kernel_string, problem_size, args, tune_params, block_size_names=block_size_names,
                             ...
                             device=0, platform=0, iterations=72,
